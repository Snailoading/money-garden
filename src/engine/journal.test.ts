...
describe("filterTransactions", () => {
  ...
  it("text is trimmed; whitespace-only text is a no-op", () => {
    expect(ids(filterTransactions(ledger, { text: "  ramen  " }))).toEqual(["a"]);
    expect(ids(filterTransactions(ledger, { text: "   " }))).toEqual(["a", "b", "c", "d", "e"]);
  });
  ...
  it(`saving rows match "${SAVING_LABEL}" (the empty-note display fallback); others don't`, () => {
    expect(ids(filterTransactions(ledger, { text: "goal contribution" }))).toEqual(["d"]);
    expect(ids(filterTransactions(ledger, { text: "CONTRIB" }))).toEqual(["d"]);
  });
  ...
  it("type filter selects exactly that type", () => {
    expect(ids(filterTransactions(ledger, { type: "expense" }))).toEqual(["a", "b", "e"]);
    expect(ids(filterTransactions(ledger, { type: "income" }))).toEqual(["c"]);
    expect(ids(filterTransactions(ledger, { type: "saving" }))).toEqual(["d"]);
  });
  ...
  it("date bounds are inclusive; from > to yields empty", () => {
    expect(ids(filterTransactions(ledger, { from: "2026-06-10" }))).toEqual(["b", "d", "e"]);
